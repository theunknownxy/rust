...
use rustc_target::spec::{
    PanicStrategy, RelocModel, RelroLevel, SanitizerSet, Target, XrayModeSet,
};
...
fn link_xray(sess: &Session, crate_type: CrateType, linker: &mut dyn Linker) {
    if crate_type != CrateType::Executable || !sess.opts.debugging_opts.xray_instrument {
        return;
    }

    let xray_modes = sess.opts.debugging_opts.xray_modes;
